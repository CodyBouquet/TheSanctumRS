...
client.on("interactionCreate", async interaction => {
  try {
    ...
    // Button interactions (LFG join/leave)
    ...
    // Modal submissions (LFG date/day/time + notes)
    if (interaction.isModalSubmit() && interaction.customId.startsWith("lfg_modal_")) {
      const user = interaction.user;
      const temp = groups.get(`temp_${user.id}`);
      if (!temp) return interaction.reply({ content: "❌ LFG session expired. Please run /lfg again.", ephemeral: true });
      ...
      if (
        ...
        return interaction.reply({ content: "❌ Invalid input. Check month, day, or time format (HHMM).", ephemeral: true });
      }
      ...
      const dateCandidate = new Date(Date.UTC(year, monthNum - 1, day, parseInt(hour), parseInt(minute)));
      if (dateCandidate < now) year += 1; // if already passed this year, schedule next year
      ...
      const join = new ButtonBuilder().setCustomId("lfg_join").setLabel("Join").setStyle(ButtonStyle.Success);
      const leave = new ButtonBuilder().setCustomId("lfg_leave").setLabel("Leave").setStyle(ButtonStyle.Danger);
      const row = new ActionRowBuilder().addComponents(join, leave);

      const msg = await interaction.reply({ embeds: [embed], components: [row], fetchReply: true });
      groups.set(msg.id, { activity, slots, players: [user.id], dateTime, notes });
    }
